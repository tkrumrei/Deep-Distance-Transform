...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def _train_net(self, train_data, train_labels, 
        ...
        for iepoch in range(self.n_epochs):
            ...
            if iepoch%2==0: # HIER HABE ICH GEÄNDERT iepoch%10=0 or iepoch=5 ###########################################################
                lavg = lavg / nsum
                if test_data is not None:
                    lavgt, nsum = 0., 0
                    np.random.seed(42)
            ...
            if save_path is not None:
                if train_loss < smallest_loss: # HIER GEÄNDERT iepoch==self.n_epochs-1 or iepoch%save_every==1: #################################################
                    smallest_loss = train_loss # HIER GEÄNDERT
                    file_name = "Best_Model" # HIER GEÄNDERT
                    ''' # HIER RAUSGENOMMEN
